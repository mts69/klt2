/*********************************************************************
 * selectGoodFeatures_cuda.cu
 *
 * CUDA version with bitonic sort instead of quicksort
 *********************************************************************/
 ...
 * 
 * Bitonic sort kernel for sorting feature points by eigenvalue
 * Each point consists of 3 integers: x, y, eigenvalue
 */

__device__ void swap3(int *data, int i, int j) {
    int temp;
    temp = data[3*i];     data[3*i] = data[3*j];     data[3*j] = temp;
    temp = data[3*i+1];   data[3*i+1] = data[3*j+1]; data[3*j+1] = temp;
    temp = data[3*i+2];   data[3*i+2] = data[3*j+2]; data[3*j+2] = temp;
}

__global__ void bitonic_sort_step(int *data, int n, int step, int stage) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    int pair_distance = 1 << (stage - 1);
    int block_width = 2 * pair_distance;
    
    if (tid >= n) return;
    
    int direction = ((tid / pair_distance) % 2) == 0 ? 1 : 0;
    
    if (direction == 1) {
        // Ascending order
        if (data[3*tid+2] > data[3*(tid + pair_distance)+2]) {
            swap3(data, tid, tid + pair_distance);
        }
    } else {
        // Descending order  
        if (data[3*tid+2] < data[3*(tid + pair_distance)+2]) {
...
__global__ void bitonic_merge_step(int *data, int n, int step) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    int pair_distance = 1 << step;
    
    if (tid >= n || tid + pair_distance >= n) return;
...
void _bitonic_sort(int *pointlist, int n) {
    if (n <= 1) return;
    
    int *d_data;
    size_t size = n * 3 * sizeof(int);
    
    // Allocate device memory
    cudaMalloc(&d_data, size);
    cudaMemcpy(d_data, pointlist, size, cudaMemcpyHostToDevice);
    ...
    for (int stage = 1; stage < n; stage <<= 1) {
        for (int step = stage - 1; step >= 0; step--) {
            bitonic_sort_step<<<gridSize, blockSize>>>(d_data, n, step, stage);
    ...
    for (int step = 0; step < n; step <<= 1) {
        bitonic_merge_step<<<gridSize, blockSize>>>(d_data, n, step);
        cudaDeviceSynchronize();
    }
    
    // Copy result back to host
    cudaMemcpy(pointlist, d_data, size, cudaMemcpyDeviceToHost);
    cudaFree(d_data);
}
...
/*********************************************************************
 * _sortPointList - CUDA version using bitonic sort
 */
...
#else
  _bitonic_sort(pointlist, npoints);
#endif
}
  ...
  selectionMode mode)
{
  ...
  /* Sort the features using CUDA bitonic sort */
